...
class BaseDataLakeGlueRole(iam.Role):
    ...
    def add_policy(self):
        policy = iam.Policy(
            self,
            id=f"iam-{self.deploy_env}-glue-data-lake-{self.layer.value}-policy",
            policy_name=f"iam-{self.deploy_env}-glue-data-lake-{self.layer.value}-policy",
            statements=[
                iam.PolicyStatement(
    ...
    def add_instance_profile(self):
        iam.CfnInstanceProfile(
            self,
            id=f"iam-{self.deploy_env}-glue-data-lake-{self.layer.value}-instance-profile",
            instance_profile_name=f"iam-{self.deploy_env}-glue-data-lake-{self.layer.value}-instance-profile",
...
class BaseGlueCrawler(glue.CfnCrawler):
    def __init__(
        ...
        **kwargs,
        ...
        self.obj_name = f"glue-{self.deploy_env}-{self.data_lake_bucket.layer.value}-{self.table_name}-crawler"
        super().__init__(
            scope,
            id=self.obj_name,
            name=self.obj_name,
...
class OrdersTable(glue.Table):
    def __init__(
        ...
        **kwargs,
        ...
        self.deploy_env = self.glue_database.deploy_env
        self.data_lake_bucket = self.glue_database.data_lake_bucket
        self.obj_name = f"glue-{self.deploy_env}-orders-table"
        super().__init__(
            scope,
            self.obj_name,
            table_name="orders",
