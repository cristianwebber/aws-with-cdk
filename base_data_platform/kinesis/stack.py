...
class RawKinesisRole(iam.Role):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            scope,
            id=f"iam-{self.deploy_env}-data-lake-raw-firehose-role",
    ...
    def add_policy(self):
        policy = iam.Policy(
            self,
            id=f"iam-{self.deploy_env}-data-lake-raw-firehose-policy",
            policy_name=f"iam-{self.deploy_env}-data-lake-raw-firehose-policy",
            statements=[
                iam.PolicyStatement(
                    actions=[
