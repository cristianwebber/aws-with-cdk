...
class AirflowStack(core.Stack):
    def __init__(
        ...
        **kwargs,
        ...
        with ZipFile(
            "base_data_plataform/airflow_stack/resources.zip", "w"
        ) as zipObj2:
            zipObj2.write(
                "base_data_plataform/airflow_stack/requirements.txt",
                arcname="requirements.txt",
            )
            for file in os.listdir("base_data_plataform/airflow_stack/dags"):
                zipObj2.write(
                    f"base_data_plataform/airflow_stack/dags/{file}",
                    arcname=f"dags/{file}",
                )
        ...
        self.dag_upload = s3deploy.BucketDeployment(
            ...
            sources=[
                s3deploy.Source.asset(
                    "base_data_plataform/airflow_stack/resources.zip"
                )
            ],
        )
